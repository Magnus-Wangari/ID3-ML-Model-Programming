...
delete_row=[]
for row,value in delete_rows[selected_root_node].items():
    if value==0 or value==1:#arrest values with extreem entopies
        delete_row.append(row)#store them in a list
...
#plotting the graph
G = nx.DiGraph()
edges=[['weight','engine'],['engine','turbo'],['turbo','fueleco'],['fueleco','fast'],['weight','deleted'],['engine','deleted'],['turbo','deleted'],['fueleco','deleted'],]
"""
G.add_edge('weight', 'engine',weight='average')
G.add_edge('weight', 'engine',string='heavy')
G.add_edge('weight', 'deleted',string='light')
G.add_edge('engine', 'turbo',string='large')
G.add_edge('engine', 'deleted',string='small')
G.add_edge('turbo', 'deleted',string='yes')
G.add_edge('turbo', 'fueleco',string='bad')
G.add_edge('fueleco', 'fast',string='bad')
G.add_edge('fueleco', 'deleted',string='average')
"""
...
nx.draw_networkx_nodes(G, pos,
                       node_color='pink', nodelist=['fast','turbo','fueleco','engine','weight','deleted'])
...
nx.draw_networkx_edge_labels(G,pos, font_size=10, font_family='sans-serif',edge_labels={('weight','engine'):'heavy&average',('weight','deleted'):'light',('engine','deleted'):'small',('engine','turbo'):'large',('turbo','deleted'):'yes',('turbo','fueleco'):'no',('fueleco','fast'):'bad',('fueleco','deleted'):'average'},font_color='black')
